...
Outputs:
    x, y, z: signed 16-bit integer of x, y, and z values
"""
def read3axis(I2CPort, address, reg, cal):
    bus = SMBus(I2CPort)
...
class IMU:
    ...
    def initializeXM(self):      
        bus = SMBus(self.I2CPort)
        resp = bus.read_i2c_block_data(self.XM.ADDRESS,self.XM.WHO_AM_I,1)[0]
        if resp != self.XM.WHO_AM_I_OK:
            raise ValueError('Accelerometer init failed')

    def initializeMAG(self):      
        bus = SMBus(self.I2CPort)
        resp = bus.read_i2c_block_data(self.MA.ADDRESS,self.MA.WHO_AM_I,1)[0]
    ...
    def enable_accel(self):
        bus = SMBus(self.I2CPort)
        bus.write_i2c_block_data(self.XM.ADDRESS,self.XM.CTRL_REG5_XL,[0x38]) # 3 axis enable 
        bus.write_i2c_block_data(self.XM.ADDRESS,self.XM.CTRL_REG6_XL,[0xC0]) # 408 Hz, 952 ODR
        bus.write_i2c_block_data(self.XM.ADDRESS,self.XM.CTRL_REG8,[0x04])    # multiple reads enable

    # Enables the gyro in normal mode on all axes
    def enable_gyro(self):
        bus = SMBus(self.I2CPort)
        bus.write_i2c_block_data(self.XM.ADDRESS,self.XM.CTRL_REG4,[0x38])   # 3 axis enable
        bus.write_i2c_block_data(self.XM.ADDRESS,self.XM.CTRL_REG1_G,[0xC3]) # 100Hz 952 ODR
        bus.write_i2c_block_data(self.XM.ADDRESS,self.XM.CTRL_REG8,[0x04])   # multiple reads enable

    # Enables the mag continuously on all axes
    def enable_mag(self):
        bus =  SMBus(self.I2CPort)
        bus.write_i2c_block_data(self.MA.ADDRESS,self.MA.CTRL_REG1_M,[0x7C]) 
        bus.write_i2c_block_data(self.MA.ADDRESS,self.MA.CTRL_REG3_M,[0x00]) # continous
    ...
    def accel_range(self,AR="2G"):
        try:
            ...
            bus = SMBus(self.I2CPort)
            accelReg = bus.read_i2c_block_data(self.XM.ADDRESS,self.XM.CTRL_REG6_XL,1)[0]
            accelReg |= Arange
            bus.write_i2c_block_data(self.XM.ADDRESS,self.XM.CTRL_REG6_XL,[accelReg])   
    ...
    def mag_range(self,MR="2GAUSS"):
        try:
            ...
            bus =  SMBus(self.I2CPort)
            magReg = bus.read_i2c_block_data(self.MA.ADDRESS,self.MA.CTRL_REG2_M,1)[0]              
            magReg &= ~(0b01100000)
            magReg |= Mrange
            bus.write_i2c_block_data(self.MA.ADDRESS,self.MA.CTRL_REG2_M,[magReg])   
    ...
    def gyro_range(self,GR="245DPS"):
        try:
            ...
            bus = SMBus(self.I2CPort)
            gyroReg = bus.read_i2c_block_data(self.XM.ADDRESS,self.XM.CTRL_REG1_G,1)[0]          
            gyroReg &= ~(0b00011000)
            gyroReg |= Grange
            bus.write_i2c_block_data(self.XM.ADDRESS,self.XM.CTRL_REG1_G,[gyroReg])               
    ...
    def read_gyro(self):
        ...
        self.gx, self.gy, self.gz = read3axis(self.I2CPort, self.XM.ADDRESS, self.XM.OUT_X_L_G, cal)  
    
    # Reads and calibrates the temperature in degrees C
    def readTemp(self):
        bus = SMBus(self.I2CPort)
        tempdata = bus.read_i2c_block_data(self.XM.ADDRESS,0x80 | self.XM.OUT_TEMP_L,2)
